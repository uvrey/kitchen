...
class MFirstSet(m.Scene):
    ...
    # add the first set titles to the canvas
        self.cfg.manim_firstset_lead[production] = m.Tex("First(" + production + "):", color = config.config.get_opp_col()
                        ...
                        if re.match(RE_TERMINAL, current_item):
                            ...
                            for ps in pstack:
                                if current_item not in self.firstset[ps]:
                                    ...
                                    new_element = m.Tex(
                                        mg.to_tex(terminal_to_write), color=m.TEAL).scale(TEXT_SCALE)
